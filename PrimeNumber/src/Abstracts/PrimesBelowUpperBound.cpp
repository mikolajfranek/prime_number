...
namespace Abstracts {
	...
	vector<Elements::PrimeOfQuadraticResidue*> *PrimesBelowUpperBound::GetPrimesOfQuadraticResidue(mpz_t m0, vector<unsigned long long>* VP){
		...
		for(unsigned long long vp : *VP){
			if(vp != 2){
				...
				if(mpz_legendre(m0, primeOfQuadraticResidue->vp) == 1){
					Solver::TonelliShanks::Solve(m0, primeOfQuadraticResidue->vp, primeOfQuadraticResidue->m7, primeOfQuadraticResidue->m8);
