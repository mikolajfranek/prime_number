...
namespace Factorization {
	...
	void QuadraticSieve::Factor(string input){
		...
		//declare
		mpz_t m3, m4, m5, m6, r0;

		//init
		mpz_set_str(this->m0, input.c_str(), 10);
		mpz_inits(m3, m4, m5, m6, r0, NULL);
						...
						if(isPowerOfNumber == false){
							...
							while(foundSmooth <= minimumSmooth){
								...
								for(long long i = beginOfInterval; i < endOfInterval; i++){
									sieve.insert(pair<string, Elements::ElementOfQuadraticSieve*>(to_string(i), new Elements::ElementOfQuadraticSieve(i, this->m0, m3, primesOfQuadraticResidue->size()+1)));
									sieve.insert(pair<string, Elements::ElementOfQuadraticSieve*>(to_string(-i), new Elements::ElementOfQuadraticSieve(-i, this->m0, m3, primesOfQuadraticResidue->size()+1)));
								}
								...
								if(accumulate(matrix[i].begin(), matrix[i].end(), 0) == 0){
									...
									for(unsigned long long j = 0; j < foundSmooth; j++){
										if(identity[i][j] == true){
											...
											mpz_mul(m5, m5, smoothNumbers[j]->element);
										}
									}
									mpz_sqrt(m4, m4);
									mpz_sqrt(m5, m5);
									mpz_sub(m6, m4, m5);
									mpz_mod(m6, m6, this->m0);
									if(mpz_cmp_ui(m6, 0) != 0){
										mpz_add(m6, m4, m5);
										mpz_mod(m6, m6, this->m0);
										if(mpz_cmp_ui(m6, 0) != 0){
											mpz_sub(m6, m4, m5);
											mpz_abs(m6, m6);
											mpz_gcd(this->m1, m6, this->m0);
											mpz_add(m6, m4, m5);
											mpz_gcd(this->m2, m6, this->m0);
										}
									}
								}
							}
		...
		//clear
		mpz_clears(m3, m4, m5, m6, r0, NULL);
	}
